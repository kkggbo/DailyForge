...
public enum ErrorCode {
    ...
    EXERCISE_NOT_FOUND("EXERCISE_NOT_FOUND", "exercise not found", HttpStatus.NOT_FOUND),
    ...
    CYCLE_TEMPLATE_STRUCTURE_TYPE_INVALID("CYCLE_TEMPLATE_STRUCTURE_TYPE_INVALID",
            "cycle template structure type is invalid", HttpStatus.BAD_REQUEST),
    CYCLE_TEMPLATE_ITEM_INVALID("CYCLE_TEMPLATE_ITEM_INVALID", "cycle template item is invalid",
            HttpStatus.BAD_REQUEST),
    CYCLE_TEMPLATE_ITEM_COUNT_INVALID("CYCLE_TEMPLATE_ITEM_COUNT_INVALID", "cycle template item count is invalid",
            HttpStatus.BAD_REQUEST),
    CYCLE_TEMPLATE_METRIC_KEY_INVALID("CYCLE_TEMPLATE_METRIC_KEY_INVALID", "cycle template metric key is invalid",
            HttpStatus.BAD_REQUEST),
    CYCLE_TEMPLATE_METRIC_DUPLICATE("CYCLE_TEMPLATE_METRIC_DUPLICATE",
            "cycle template metric key is duplicated", HttpStatus.BAD_REQUEST),
    CYCLE_TEMPLATE_METRIC_VALUE_INVALID("CYCLE_TEMPLATE_METRIC_VALUE_INVALID",
            "cycle template metric value is invalid", HttpStatus.BAD_REQUEST),
    ...
    CYCLE_TEMPLATE_OVERWRITE_CONFIRM_REQUIRED("CYCLE_TEMPLATE_OVERWRITE_CONFIRM_REQUIRED",
            "active template update overwrite confirmation is required", HttpStatus.CONFLICT),
    ...
    WORKOUT_ACTIVE_CYCLE_NOT_FOUND("WORKOUT_ACTIVE_CYCLE_NOT_FOUND", "active workout cycle not found",
            HttpStatus.CONFLICT),
    WORKOUT_CYCLE_COMPLETED("WORKOUT_CYCLE_COMPLETED", "workout cycle has been completed",
            HttpStatus.CONFLICT),
    WORKOUT_CYCLE_RESTART_FORBIDDEN("WORKOUT_CYCLE_RESTART_FORBIDDEN",
            "workout cycle cannot be restarted in its current state", HttpStatus.CONFLICT),
    WORKOUT_AI_ANALYSIS_COMPLETED_CYCLE_REQUIRED("WORKOUT_AI_ANALYSIS_COMPLETED_CYCLE_REQUIRED",
            "workout cycle must be completed before requesting ai analysis", HttpStatus.CONFLICT),
    WORKOUT_DAY_OUT_OF_RANGE("WORKOUT_DAY_OUT_OF_RANGE", "workout day index is out of range",
            HttpStatus.BAD_REQUEST),
    WORKOUT_CURRENT_DAY_SESSION_CONFLICT("WORKOUT_CURRENT_DAY_SESSION_CONFLICT",
            "current workout day session conflicts with cycle progress", HttpStatus.CONFLICT),
    WORKOUT_SESSION_NOT_FOUND("WORKOUT_SESSION_NOT_FOUND", "workout session not found", HttpStatus.NOT_FOUND),
    WORKOUT_SESSION_STATUS_INVALID("WORKOUT_SESSION_STATUS_INVALID", "workout session status is invalid",
            HttpStatus.CONFLICT),
    WORKOUT_SESSION_EDIT_FORBIDDEN("WORKOUT_SESSION_EDIT_FORBIDDEN", "workout session cannot be edited",
            HttpStatus.CONFLICT),
    WORKOUT_SESSION_COMPLETE_FORBIDDEN("WORKOUT_SESSION_COMPLETE_FORBIDDEN",
            "workout session cannot be completed for the current cycle", HttpStatus.CONFLICT),
    WORKOUT_SESSION_EXERCISE_INVALID("WORKOUT_SESSION_EXERCISE_INVALID",
            "workout session exercise payload does not match the session snapshot", HttpStatus.BAD_REQUEST),
    WORKOUT_EXERCISE_STATUS_REQUIRED("WORKOUT_EXERCISE_STATUS_REQUIRED",
            "every workout exercise requires a completion status", HttpStatus.BAD_REQUEST),
    WORKOUT_EXERCISE_STATUS_INVALID("WORKOUT_EXERCISE_STATUS_INVALID", "workout exercise status is invalid",
            HttpStatus.BAD_REQUEST),
    WORKOUT_FAILURE_REASON_INVALID("WORKOUT_FAILURE_REASON_INVALID", "workout failure reason is invalid",
            HttpStatus.BAD_REQUEST),
    WORKOUT_ITEM_INVALID("WORKOUT_ITEM_INVALID", "workout exercise item payload does not match the session snapshot",
            HttpStatus.BAD_REQUEST),
    WORKOUT_METRIC_INVALID("WORKOUT_METRIC_INVALID", "workout metric payload does not match the session snapshot",
            HttpStatus.BAD_REQUEST),
    WORKOUT_METRIC_VALUE_INVALID("WORKOUT_METRIC_VALUE_INVALID", "workout metric value is invalid",
            HttpStatus.BAD_REQUEST),
