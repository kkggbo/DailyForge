...
import com.dailyforge.modules.aicoach.infrastructure.ai.context.NextCycleGenerationContextBuilder;
import com.dailyforge.modules.aicoach.infrastructure.ai.prompt.NextCycleGenerationPromptBuilder;
...
@Component
public class NextCycleGenerationExecutor implements AiScenarioExecutor {

    private final ObjectMapper objectMapper;
    private final AiCoachProperties aiCoachProperties;
    private final NextCycleGenerationContextBuilder contextBuilder;
    private final NextCycleGenerationPromptBuilder promptBuilder;
    ...
    public NextCycleGenerationExecutor(
            ObjectMapper objectMapper,
            AiCoachProperties aiCoachProperties,
            NextCycleGenerationContextBuilder contextBuilder,
            NextCycleGenerationPromptBuilder promptBuilder,
    ...
    public void execute(AiTaskRecordEntity task) {
        ...
        NextCycleGenerationContext context = contextBuilder.build(task.getUserId(), request);
        ...
        aiTemplateGenerationService.persistNextCycleSuccessfulResult(
                task.getId(),
                write(context),
                validatedResult);
    }
    ...
    private TemplateGenerationValidatedResult validateWithRepair(
        ...
        for (int attempt = 0; attempt <= aiCoachProperties.getMaxRepairAttempts(); attempt++) {
            try {
                return aiOutputValidationDomainService.validateTemplateGeneration(currentJson, request.cycleLength());
