...
后续 DailyForge 开发默认采用“用户只与主控会话沟通，主控会话负责调度 Subagent”的工作方式。

主控会话必须承担以下长期职责：

1. 每轮开始时读取 `AGENTS.md` 与本规范，并结合最新 PRD、接口文档、改造清单判断是否需要启用 Subagent。
2. 当任务进入实现、测试、审查、提交前门禁等阶段时，主动使用 `tool_search` 查找可用多 Agent 工具，并优先按本规范创建对应 Subagent。
3. 创建 Subagent 时，必须明确适用 skill、任务目标、写入范围、参考文档、禁止修改区域和返回要求。
4. 用户不需要再手动把同一份需求、接口文档或改造清单转发给前端、后端、测试、审查或 Git 管家 Agent；这些上下文由主控会话负责整理和下发。
5. 如果当前会话没有暴露可用的 Subagent 调度工具，主控会话必须显式说明，并按同一 Pipeline 在本地完成等价检查，不得跳过契约校验、测试、审查或 Git Steward 门禁。
6. 主控会话负责最终收敛各 Agent 输出，判断是否允许进入联调、提交或推送，并向用户给出清晰结论。
### 8.2 Subagent 实例复用与关闭规则

为避免同职责 Agent 实例不断累积、偏离“固定角色位”的设计目标，DailyForge 项目默认采用以下调度约束：

1. 同角色默认单实例复用。主控会话在创建新 Subagent 前，必须先检查当前会话内是否已有同职责实例可复用；若已有实例仍适合承接当前任务，则优先复用，不重复创建。
2. 只有在并行确实有价值时，才允许创建第二个同职责实例。允许场景应同时满足：
   - 两个任务彼此独立
   - 写入范围互斥或均为只读
   - 并行推进能明显缩短总耗时，而不是仅仅为了形式上“按角色执行”
3. 以下情况应优先由主控会话本地处理，而不是额外新建同职责实例：
   - 当前阻塞问题规模很小，主控会话可直接解决
   - 只是读取少量文件、确认当前状态、做轻量契约或门禁检查
   - 只是因为旧实例已经完成，但其返回结果已经足够使用
4. 阶段结束后必须及时关闭实例。满足以下任一条件时，应执行 `close_agent`，不继续保留：
   - 该角色本轮任务已经完成，且短期内没有继续追问或继续写入的需要
   - 主控会话已完成结果收敛，不再需要等待该实例返回
   - 该实例所属阶段已经结束，例如专项测试结束、代码审查结束、提交与推送完成
5. 若主控会话决定保留已完成实例，必须有明确复用理由，例如马上还要基于同一上下文继续追问；否则默认关闭。
6. 主控会话在阶段汇总时，应尽量说明本轮哪些角色被复用、哪些角色被新建，以及新建理由，保证用户能够理解当前实例数量是否符合预期。
---
