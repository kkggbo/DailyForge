...
const secondaryButton = "inline-flex rounded-full border border-white/10 bg-white/8 px-5 py-3 text-sm font-semibold text-stone-100 transition hover:bg-white/12";
...
export function WorkoutPage() {
  ...
  const [completedCycleOnCurrentDay, setCompletedCycleOnCurrentDay] = useState(false);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    async function load() {
      setIsLoading(true); setPageError(null); setActionError(null); setMessage(null); setCompletedCycleOnCurrentDay(false);
      try {
        const nextWorkspace = await getWorkspace(token);
        if (cancelled) return;
        setWorkspace(nextWorkspace); setDetail(null); setSelectedDayIndex(nextWorkspace.defaultDayIndex);
        if (nextWorkspace.workspaceState === "active" && nextWorkspace.defaultDayIndex !== null) {
          try {
            const initialized = await initializeCurrentDay(token);
            if (!cancelled) setDetail(initialized.day);
          } catch (error) {
            if (error instanceof ApiRequestError && error.code === "WORKOUT_CYCLE_COMPLETED") {
              const refreshed = await getWorkspace(token);
              if (!cancelled) { setWorkspace(refreshed); setSelectedDayIndex(refreshed.defaultDayIndex); }
            } else throw error;
          }
        }
        void loadRecent(token, cancelled);
      } catch (error) { if (!cancelled) setPageError(errorMessage(error, "加载训练工作台失败，请稍后重试。")); }
  ...
  async function loadRecent(token: string, cancelled = false) {
    setRecentError(null);
    try { const response = await getRecent(token); if (!cancelled) setRecent(response); }
    catch (error) { if (!cancelled) setRecentError(errorMessage(error, "加载最近训练记录失败。")); }
  }

  async function selectDay(dayIndex: number) {
    if (!accessToken || dayIndex === selectedDayIndex) return;
    const requestSequence = ++dayViewRequestSequence.current;
    setSelectedDayIndex(dayIndex); setIsLoadingDay(true); setPageError(null); setActionError(null); setMessage(null); setCompletedCycleOnCurrentDay(false);
    try {
      const nextDetail = await getDay(accessToken, dayIndex);
      if (requestSequence === dayViewRequestSequence.current) setDetail(nextDetail);
    } catch (error) {
      if (requestSequence === dayViewRequestSequence.current) setPageError(errorMessage(error, "加载该训练日失败，请稍后重试。"));
    } finally {
      if (requestSequence === dayViewRequestSequence.current) setIsLoadingDay(false);
    }
  }

  async function save(payload: SavePayload) {
    if (!accessToken || !detail?.session) return;
    setIsSaving(true); setActionError(null); setMessage(null);
    try { const response = await saveSession(accessToken, detail.session.sessionId, payload); setMessage(`已保存于 ${formatTime(response.savedAt)}。`); }
    catch (error) { setActionError(errorMessage(error, "保存训练记录失败，请稍后重试。")); }
  ...
  async function complete(payload: SavePayload) {
    if (!accessToken || !detail?.session) return;
    const requestSequence = ++dayViewRequestSequence.current;
    setIsCompleting(true); setActionError(null); setMessage(null);
    try {
      const response = await completeSession(accessToken, detail.session.sessionId, payload);
      if (requestSequence !== dayViewRequestSequence.current) return;
      ...
      setMessage(response.cycleRunStatus === "completed" ? "本 Day 已完成，当前循环也已完成。刷新或重新进入后可选择下一步。" : "本 Day 已完成。当前页面保留在这条只读记录上。");
      void loadRecent(accessToken);
    } catch (error) {
      if (requestSequence === dayViewRequestSequence.current) setActionError(errorMessage(error, "完成训练打卡失败，请稍后重试。"));
    } finally { setIsCompleting(false); }
  }

  async function restart() {
    if (!accessToken) return;
    setIsRestarting(true); setActionError(null); setMessage(null);
    try {
      await restartCycle(accessToken);
      const nextWorkspace = await getWorkspace(accessToken);
      setWorkspace(nextWorkspace); setSelectedDayIndex(nextWorkspace.defaultDayIndex); setCompletedCycleOnCurrentDay(false);
      if (nextWorkspace.workspaceState === "active" && nextWorkspace.defaultDayIndex !== null) setDetail((await initializeCurrentDay(accessToken)).day);
    } catch (error) { setActionError(errorMessage(error, "重新开始当前循环失败，请稍后重试。")); }
  ...
  return <section className="space-y-8">
    ...
    {workspace.workspaceState === "active" ? <>
      ...
      {isLoadingDay ? <Loading label="正在加载训练日..." /> : detail ? <DayPanel detail={detail} isSaving={isSaving} isCompleting={isCompleting} error={actionError} onSave={save} onComplete={complete} /> : <Notice>当前训练日尚未加载完成。</Notice>}
    </> : null}
    <RecentList data={recent} error={recentError} />
  </section>;
}

function DayPanel({ detail, isSaving, isCompleting, error, onSave, onComplete }: { detail: DayDetail; isSaving: boolean; isCompleting: boolean; error: string | null; onSave: (payload: SavePayload) => void; onComplete: (payload: SavePayload) => void }) {
  const editableSession = detail.viewMode === "editable" ? detail.session : null;
  return <section className="space-y-5"><div className="rounded-[32px] border border-white/10 bg-black/20 p-6"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm uppercase tracking-[0.22em] text-amber-300">Day {detail.dayIndex} · {detail.isRestDay ? "Rest Day" : "Training Day"}</p><h2 className="mt-2 text-3xl font-semibold text-white">{detail.dayName}</h2><p className="mt-2 text-stone-300">{detail.viewMode === "preview" ? "未来训练计划预览，不会创建 session。" : detail.viewMode === "readonly" ? "已完成记录仅供查看，不能编辑。" : "必要时记录与计划不一致的实际值后，可手动保存或完成打卡。"}</p></div><span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-sm text-stone-300">{detail.viewMode === "editable" ? "可编辑" : detail.viewMode === "readonly" ? "只读" : "计划预览"}</span></div></div>{editableSession ? <SessionEditor session={editableSession} isSaving={isSaving} isCompleting={isCompleting} error={error} onSave={onSave} onComplete={onComplete} /> : null}{detail.viewMode === "readonly" && detail.session ? <SessionReadOnly session={detail.session} /> : null}{detail.viewMode === "preview" ? <Preview exercises={detail.exercises ?? []} rest={detail.isRestDay} /> : null}</section>;
}
