...
1. DailyForge 的 8 个 Agent 角色是固定职责位，不应把“同职责重复开新实例”当作默认行为。
2. 同角色默认单实例复用。主控会话在创建新 Subagent 前，必须先检查当前会话内是否已有同职责实例可复用；能复用时优先复用，不重复创建。
3. 只有在并行确实有价值时，才允许创建第二个同职责实例。典型场景仅包括：
   - 同一职责下存在两个及以上彼此独立、写入范围互斥、且同时推进能明显缩短总耗时的任务
   - 当前同职责实例正在处理长耗时任务，而主控会话的下一步又必须并行启动另一个同职责但不共享写入范围的任务
4. 以下情况不允许为了形式上“走角色”而额外创建新实例：
   - 当前阻塞问题主控会话可直接本地解决
   - 只是读取少量文件、确认状态、执行小型门禁检查
   - 只是因为已有实例已完成但结果足够使用，没有继续交互需求
5. 阶段结束后必须及时 `close_agent`。满足以下任一条件时，不应继续保留该实例：
   - 该角色本轮任务已经完成且短期内不再复用
   - 主控会话已完成结果收敛，不再需要等待该实例返回
   - 该实例对应的阶段已经结束，例如审查完成、提交完成、专项测试完成
6. 若确实需要保留已完成实例，也应有明确理由，例如主控会话预计紧接着继续向同一实例追问同一上下文问题；否则默认关闭。
7. 主控会话在汇报阶段结论时，应明确说明本轮哪些角色被复用、哪些角色被新建，以及新建的理由，避免同职责实例无序增长。
